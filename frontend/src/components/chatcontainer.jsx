...
const ChatContainer = ({ currentChat, currentUser, socket }) => {
    ...
    const fetchMessages = async () => {
      if (currentChat) {
        const response = await axios.post(
          `${import.meta.env.MODE === "development" ? `http://localhost:8000/api/messages/getmsg` : import.meta.env.VITE_BACKEND_URL+`/api/messages/getmsg`}`,
          {
  ...
  const handleSendMsg = async (message) => {
    const { data } = await axios.post(
      `${import.meta.env.MODE === "development" ? `http://localhost:8000/api/messages/addmsg` : import.meta.env.VITE_BACKEND_URL+`/api/messages/addmsg`}`,
      {
        from: currentUser._id,
        to: currentChat._id,
        message,
